...
mat_zombie = make_mat("Zombie", (0.6, 0.6, 0.65, 1))
mat_jeep = make_mat("Jeep_Body", (0.22, 0.28, 0.18, 1))  # olive drab
mat_jeep_dark = make_mat("Jeep_Dark", (0.12, 0.12, 0.1, 1))  # tires/details
mat_road = make_mat("Road", (0.15, 0.14, 0.13, 1))  # dark asphalt
mat_driver = make_mat("Driver", (0.35, 0.3, 0.2, 1))  # generic seated figures

# Ground (desert)
add_ground(mat_ground, size=30)

# ─── Desert Highway ───────────────────────────────────────────
# Road strip running along Y axis, centered at x=-1
add_mesh("Road", bpy.ops.mesh.primitive_cube_add, mat_road,
         (-1, 10, 0.01), (2.5, 25, 0.01))

# Road center line
mat_line = make_mat("RoadLine", (0.6, 0.55, 0.3, 1))
add_mesh("CenterLine", bpy.ops.mesh.primitive_cube_add, mat_line,
         (-1, 10, 0.02), (0.05, 25, 0.005))

# ─── Military Jeep (center-left, facing forward along +Y) ────
jeep_x, jeep_y, jeep_z = -1.5, 2.0, 0.0

# Jeep chassis
add_mesh("Jeep_Chassis", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x, jeep_y, jeep_z + 0.5), (0.9, 1.6, 0.25))

# Jeep hood (front)
add_mesh("Jeep_Hood", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x, jeep_y + 1.2, jeep_z + 0.65), (0.7, 0.6, 0.12))

# Jeep rear cargo bed (raised slightly)
add_mesh("Jeep_Cargo", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x, jeep_y - 0.8, jeep_z + 0.55), (0.85, 0.7, 0.15))

# Jeep side walls (cargo bed)
add_mesh("Jeep_SideL", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x - 0.85, jeep_y - 0.8, jeep_z + 0.75), (0.05, 0.7, 0.2))
add_mesh("Jeep_SideR", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x + 0.85, jeep_y - 0.8, jeep_z + 0.75), (0.05, 0.7, 0.2))

# Jeep rear wall
add_mesh("Jeep_Rear", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x, jeep_y - 1.5, jeep_z + 0.75), (0.85, 0.05, 0.2))

# Windshield frame
add_mesh("Jeep_Windshield", bpy.ops.mesh.primitive_cube_add, mat_jeep_dark,
         (jeep_x, jeep_y + 0.5, jeep_z + 1.05), (0.8, 0.05, 0.3))

# Wheels (4 cylinders)
for wx, wy in [(-0.95, 0.9), (0.95, 0.9), (-0.95, -0.6), (0.95, -0.6)]:
    add_mesh(f"Wheel_{wx}_{wy}", bpy.ops.mesh.primitive_cylinder_add, mat_jeep_dark,
             (jeep_x + wx, jeep_y + wy, jeep_z + 0.25), (0.25, 0.25, 0.1),
             rot=(0, math.radians(90), 0))

# ─── Gorilla — standing in rear cargo bed, arms raised (holding bat) ──
build_large_figure("Gorilla", mat_gorilla, (jeep_x, jeep_y - 0.8, jeep_z + 0.7), pose="arms_raised")

# Baseball bat (long cylinder in gorilla's hands)
mat_bat = make_mat("Bat", (0.55, 0.4, 0.2, 1))  # wooden color
add_mesh("Bat", bpy.ops.mesh.primitive_cylinder_add, mat_bat,
         (jeep_x, jeep_y - 0.8, jeep_z + 0.7 + 2.7), (0.06, 0.06, 0.7),
         rot=(0, math.radians(15), math.radians(10)))

# ─── Two seated figures in front seats ─────────────────────────
# Driver (left seat)
build_regular_male("Driver", mat_driver, (jeep_x - 0.4, jeep_y + 0.3, jeep_z + 0.5), pose="seated")

# Passenger (right seat)
build_regular_male("Passenger", mat_driver, (jeep_x + 0.4, jeep_y + 0.3, jeep_z + 0.5), pose="seated")

# ─── Zombie — far ahead on right road shoulder, shambling ─────
build_regular_male("Zombie", mat_zombie, (1.5, 18.0, 0.0), pose="standing")

# ─── Camera — behind and slightly above the jeep, looking forward ──
setup_camera(scene,
             loc=(jeep_x - 0.5, jeep_y - 5.5, jeep_z + 3.0),
             target_loc=(0, 14, 1.0),
             lens=35)

# ─── Render ────────────────────────────────────────────────────
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/zombie_mailbox/layouts/layout_000.png"
