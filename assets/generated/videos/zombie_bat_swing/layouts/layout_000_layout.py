...
vehicle_mat = make_mat("Vehicle", (0.3, 0.25, 0.2, 1))

# Ground plane (desert road)
add_ground(ground_mat, size=15)

# Vehicle bed — flat platform the gorilla stands on
add_mesh("VehicleBed", bpy.ops.mesh.primitive_cube_add, vehicle_mat,
         (0, 0, 0.3), (1.8, 1.2, 0.15))
# Vehicle side rails
add_mesh("RailL", bpy.ops.mesh.primitive_cube_add, vehicle_mat,
         (-1.8, 0, 0.6), (0.08, 1.2, 0.3))
add_mesh("RailR", bpy.ops.mesh.primitive_cube_add, vehicle_mat,
         (1.8, 0, 0.6), (0.08, 1.2, 0.3))

# Gorilla — left side of frame, fighting stance with arm extended for the swing
# Positioned on the vehicle bed (z=0.45 accounts for bed height)
build_large_figure("Gorilla", gorilla_mat, (-0.8, 0, 0.45), pose="fighting_stance")

# Bat — long cylinder extending from gorilla's right arm toward the zombie
# Angled in a swinging arc from gorilla toward zombie
add_mesh("Bat", bpy.ops.mesh.primitive_cylinder_add, bat_mat,
         (0.6, -0.5, 1.8), (0.06, 0.06, 1.2),
         rot=(math.radians(10), math.radians(-70), math.radians(15)))

# Zombie — right side of frame, at arm's length, standing at same height
# On the ground beside the vehicle (z=0 ground level, but elevated slightly
# to appear at vehicle bed height — standing on road embankment or lunging up)
build_regular_male("Zombie", zombie_mat, (2.5, -0.3, 0.3), pose="standing")

# Camera — tight behind gorilla's right shoulder, over-the-shoulder shot
# Looking past the bat toward the zombie, wide 24mm lens
setup_camera(scene,
             loc=(-1.5, 1.8, 2.4),
             target_loc=(2.5, -0.3, 1.5),
             lens=24)

# Render
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/zombie_bat_swing/layouts/layout_000.png"
