...
# Materials
ground_mat = make_mat("DesertGround", (0.2, 0.15, 0.1, 1))
road_mat = make_mat("Road", (0.08, 0.08, 0.08, 1))
line_mat = make_mat("RoadLine", (0.9, 0.8, 0.3, 1))
dot_mat = make_mat("DistantDot", (0.3, 0.3, 0.35, 1))

# Ground plane — wide desert
add_ground(ground_mat, size=30)

# Road — long thin dark rectangle stretching into the distance
add_mesh("Road", bpy.ops.mesh.primitive_cube_add, road_mat,
         (0, 30, 0.01), (1.5, 30, 0.01))

# Center dashed line markings
for i in range(20):
    y_pos = 2 + i * 3
    add_mesh(f"DashLine_{i}", bpy.ops.mesh.primitive_cube_add, line_mat,
             (0, y_pos, 0.02), (0.04, 0.5, 0.005))

# Road edge lines
add_mesh("EdgeLineL", bpy.ops.mesh.primitive_cube_add, line_mat,
         (-1.4, 30, 0.02), (0.03, 30, 0.005))
add_mesh("EdgeLineR", bpy.ops.mesh.primitive_cube_add, line_mat,
         (1.4, 30, 0.02), (0.03, 30, 0.005))

# Distant approaching object — small cube far down the road
add_mesh("DistantObject", bpy.ops.mesh.primitive_cube_add, dot_mat,
         (0, 55, 0.3), (0.15, 0.15, 0.15))

# No characters in this shot

# Camera — low angle near road level, looking down the road into the distance
setup_camera(scene, loc=(0, -2, 0.4), target_loc=(0, 50, 1.5), lens=35)

# Render
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/desert_run/layouts/layout_000.png"
