...
mat_road_line = make_mat("RoadLine", (0.85, 0.8, 0.3, 1))
mat_vehicle = make_mat("Vehicle", (0.15, 0.15, 0.18, 1))

# Ground — desert terrain
add_ground(mat_ground, size=20)

# Road surface — long flat strip stretching into distance
add_mesh("Road", bpy.ops.mesh.primitive_cube_add, mat_road,
         (0, 15, 0.01), (3.0, 18, 0.01))

# Road center line dashes
for i in range(12):
    add_mesh(f"DashLine_{i}", bpy.ops.mesh.primitive_cube_add, mat_road_line,
             (0, i * 3.0 - 2, 0.02), (0.06, 0.6, 0.005))

# Road cracks — thin dark lines on asphalt
mat_crack = make_mat("Crack", (0.08, 0.08, 0.08, 1))
add_mesh("Crack1", bpy.ops.mesh.primitive_cube_add, mat_crack,
         (0.8, 1.5, 0.015), (0.02, 1.2, 0.003),
         rot=(0, 0, math.radians(12)))
add_mesh("Crack2", bpy.ops.mesh.primitive_cube_add, mat_crack,
         (-1.1, 3.0, 0.015), (0.015, 0.9, 0.003),
         rot=(0, 0, math.radians(-8)))

# Zombie — fallen on the road in the foreground
build_regular_male("Zombie", mat_zombie, (0.5, 1.5, 0.0), pose="fallen")

# Distant vehicle shape — simple box receding on the highway
add_mesh("Vehicle_Body", bpy.ops.mesh.primitive_cube_add, mat_vehicle,
         (0, 28, 0.4), (0.5, 0.8, 0.35))
add_mesh("Vehicle_Cab", bpy.ops.mesh.primitive_cube_add, mat_vehicle,
         (0, 27.6, 0.9), (0.4, 0.5, 0.25))

# Wheels
mat_wheel = make_mat("Wheel", (0.05, 0.05, 0.05, 1))
add_mesh("Wheel_FL", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-0.55, 27.4, 0.15), (0.15, 0.15, 0.08),
...
add_mesh("Wheel_RL", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-0.55, 28.6, 0.15), (0.15, 0.15, 0.08),
         rot=(0, math.radians(90), 0))
add_mesh("Wheel_RR", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (0.55, 28.6, 0.15), (0.15, 0.15, 0.08),
         rot=(0, math.radians(90), 0))

# Camera — ground level, wide angle, looking down the road past the fallen zombie
setup_camera(scene, loc=(0.3, -2.5, 0.35), target_loc=(0, 15, 0.3), lens=24)

# Render
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/zombie_mailbox_v2/layouts/layout_001.png"
