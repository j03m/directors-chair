...
# ── Massive Desert Cliff ─────────────────────────────────────────────────────
# Large cliff wall — tall cube scaled to look like a cliff face
add_mesh("CliffMain", bpy.ops.mesh.primitive_cube_add, mat_cliff,
         (0, 8, 5), (8, 3, 5))

# Cliff top ledge — flattened cube on top of main cliff
add_mesh("CliffTop", bpy.ops.mesh.primitive_cube_add, mat_cliff,
         (0, 7, 10.2), (9, 4, 0.3))

# Additional cliff face detail — slightly offset slab for depth
add_mesh("CliffFace1", bpy.ops.mesh.primitive_cube_add, mat_cliff_dark,
         (-3, 6.5, 4), (2.5, 1.5, 4.5),
         rot=(0, 0, math.radians(3)))

add_mesh("CliffFace2", bpy.ops.mesh.primitive_cube_add, mat_cliff_dark,
         (4, 6.8, 3.5), (3, 1.2, 4),
         rot=(0, 0, math.radians(-2)))

# Rocky base debris
add_mesh("Rock1", bpy.ops.mesh.primitive_cube_add, mat_cliff_dark,
         (-5, 5, 0.4), (1.5, 1, 0.4),
         rot=(0, 0, math.radians(15)))
add_mesh("Rock2", bpy.ops.mesh.primitive_cube_add, mat_cliff_dark,
         (6, 5.5, 0.3), (1, 0.8, 0.3),
         rot=(0, 0, math.radians(-20)))
add_mesh("Rock3", bpy.ops.mesh.primitive_cube_add, mat_cliff,
         (2, 5, 0.5), (0.8, 0.6, 0.5),
         rot=(math.radians(10), 0, math.radians(30)))

# ── Three Tiny Silhouetted Figures on Cliff Edge ─────────────────────────────
# Placed on top of the cliff at z=10.5 (on the ledge), very small scale
# These are silhouettes — dark material, standing at cliff edge looking out
# Scaled down to appear tiny and distant

# Figure 1 — center
add_mesh("Figure1_Body", bpy.ops.mesh.primitive_cube_add, mat_silhouette,
         (0, 5.5, 10.9), (0.12, 0.08, 0.2))
add_mesh("Figure1_Head", bpy.ops.mesh.primitive_uv_sphere_add, mat_silhouette,
         (0, 5.5, 11.25), (0.07, 0.06, 0.07))

# Figure 2 — left of center
add_mesh("Figure2_Body", bpy.ops.mesh.primitive_cube_add, mat_silhouette,
         (-1.2, 5.5, 10.9), (0.12, 0.08, 0.2))
add_mesh("Figure2_Head", bpy.ops.mesh.primitive_uv_sphere_add, mat_silhouette,
         (-1.2, 5.5, 11.25), (0.07, 0.06, 0.07))

# Figure 3 — right of center
add_mesh("Figure3_Body", bpy.ops.mesh.primitive_cube_add, mat_silhouette,
         (1.3, 5.5, 10.9), (0.12, 0.08, 0.2))
add_mesh("Figure3_Head", bpy.ops.mesh.primitive_uv_sphere_add, mat_silhouette,
         (1.3, 5.5, 11.25), (0.07, 0.06, 0.07))

# ── Camera — far below and away, looking up at cliff top ─────────────────────
setup_camera(scene, loc=(0, -18, 2), target_loc=(0, 6, 10.5), lens=28)

# ── Render ────────────────────────────────────────────────────────────────────
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/raider_ambush/layouts/layout_013.png"
