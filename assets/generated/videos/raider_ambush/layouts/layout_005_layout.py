...
mat_scope = make_mat("Scope", (0.08, 0.08, 0.08, 1))
mat_eye = make_mat("Eye", (0.9, 0.7, 0.2, 1))

# Ground
add_ground(mat_ground)

# ── Extreme close-up of gorilla face with sniper scope ───────────────────────
# Place the gorilla head large and centered, with a scope tube at eye level.
# Since this is an extreme close-up, we only build the head and scope —
# no full body needed.

# Large gorilla head — centered at origin, scaled up for close-up framing
add_mesh("Gorilla_Head", bpy.ops.mesh.primitive_uv_sphere_add, mat_gorilla,
         (0, 0, 1.5), (0.9, 0.8, 0.75))

# Brow ridge — heavy shelf above the eyes
add_mesh("Gorilla_Brow", bpy.ops.mesh.primitive_cube_add, mat_gorilla,
         (0, -0.55, 1.75), (0.7, 0.15, 0.12))

# Jaw / muzzle — protruding lower face
add_mesh("Gorilla_Jaw", bpy.ops.mesh.primitive_cube_add, mat_gorilla,
         (0, -0.6, 1.2), (0.45, 0.25, 0.2))

# Right eye — pressed against scope (squinted, not visible)
# Left eye — visible, intense, focused
add_mesh("Gorilla_EyeL", bpy.ops.mesh.primitive_uv_sphere_add, mat_eye,
         (-0.35, -0.65, 1.6), (0.12, 0.08, 0.1))

# Sniper scope — long cylinder extending from right eye outward
add_mesh("Scope_Tube", bpy.ops.mesh.primitive_cylinder_add, mat_scope,
         (0.4, -1.5, 1.6), (0.1, 0.1, 0.8),
         rot=(math.radians(80), 0, math.radians(5)))

# Scope eyepiece — wider ring where eye meets scope
add_mesh("Scope_Eyepiece", bpy.ops.mesh.primitive_cylinder_add, mat_scope,
         (0.38, -0.75, 1.6), (0.15, 0.15, 0.08),
         rot=(math.radians(90), 0, math.radians(5)))

# Scope objective lens (front end) — slightly wider
add_mesh("Scope_Objective", bpy.ops.mesh.primitive_cylinder_add, mat_scope,
         (0.42, -2.2, 1.62), (0.14, 0.14, 0.06),
         rot=(math.radians(80), 0, math.radians(5)))

# Helmet horn hints on top of head (viking horned helmet)
mat_horn = make_mat("Horn", (0.35, 0.3, 0.2, 1))
add_mesh("Horn_L", bpy.ops.mesh.primitive_cone_add, mat_horn,
         (-0.5, -0.1, 2.2), (0.08, 0.08, 0.3),
         rot=(0, math.radians(30), 0))
add_mesh("Horn_R", bpy.ops.mesh.primitive_cone_add, mat_horn,
         (0.5, -0.1, 2.2), (0.08, 0.08, 0.3),
         rot=(0, math.radians(-30), 0))

# Helmet rim across forehead
mat_helmet = make_mat("Helmet", (0.3, 0.25, 0.15, 1))
add_mesh("Helmet_Rim", bpy.ops.mesh.primitive_cube_add, mat_helmet,
         (0, -0.55, 1.9), (0.75, 0.1, 0.08))

# ── Camera — directly in front at face level, very close ─────────────────────
setup_camera(scene, loc=(0, -3.5, 1.55), target_loc=(0, 0, 1.55), lens=50)

# ── Render ───────────────────────────────────────────────────────────────────
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/raider_ambush/layouts/layout_005.png"
