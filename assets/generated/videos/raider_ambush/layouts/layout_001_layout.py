...
mat_cliff = make_mat("Cliff", (0.35, 0.28, 0.2, 1))
mat_rifle = make_mat("Rifle", (0.1, 0.1, 0.1, 1))

# Ground plane
add_ground(mat_ground)

# Cliff edge platform — raised rocky ledge
...
add_mesh("CliffRock1", bpy.ops.mesh.primitive_cube_add, mat_cliff,
         (2.5, 3.5, 0.25), (0.8, 0.5, 0.25),
         rot=(0, 0, math.radians(15)))
add_mesh("CliffRock2", bpy.ops.mesh.primitive_cube_add, mat_cliff,
         (-2.0, 3.8, 0.2), (0.6, 0.4, 0.2),
         rot=(0, 0, math.radians(-10)))

# Cranial — prone on the left, using "fallen" pose oriented toward cliff edge
# Fallen pose lays the figure flat, facing forward (toward +x)
# We rotate the whole group to face the cliff edge (toward +y)
build_regular_male("Cranial", mat_cranial, (-1.2, 2.0, 0.15), pose="fallen")

# Gorilla — prone on the right, large figure using "fallen" pose
build_large_figure("Gorilla", mat_gorilla, (1.2, 2.0, 0.15), pose="fallen")
...
add_mesh("Rifle_Cranial", bpy.ops.mesh.primitive_cylinder_add, mat_rifle,
         (-1.2, 3.2, 0.25), (0.03, 0.03, 0.7),
         rot=(math.radians(88), 0, math.radians(5)))
add_mesh("Rifle_Gorilla", bpy.ops.mesh.primitive_cylinder_add, mat_rifle,
         (1.2, 3.2, 0.3), (0.04, 0.04, 0.8),
         rot=(math.radians(88), 0, math.radians(-5)))

# Camera — low angle, ground level, slightly to the side to show both in profile
setup_camera(scene,
             loc=(3.5, -1.5, 0.4),
             target_loc=(0, 2.5, 0.3),
             lens=28)

# Render
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/raider_ambush/layouts/layout_001.png"
