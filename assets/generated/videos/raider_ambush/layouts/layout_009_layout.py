...
mat_fallen1 = make_mat("Fallen1", (0.25, 0.18, 0.1, 1))
mat_fallen2 = make_mat("Fallen2", (0.2, 0.35, 0.5, 1))
mat_fallen3 = make_mat("Fallen3", (0.3, 0.4, 0.2, 1))
mat_debris = make_mat("Debris", (0.3, 0.25, 0.15, 1))
mat_weapon = make_mat("Weapon", (0.4, 0.4, 0.4, 1))

# Ground
add_ground(mat_ground)

# Gale — mid-run lunging forward toward a weapon on the ground
# Positioned center-frame, leaning forward in fighting_stance to convey lunging motion
build_regular_female("Gale", mat_gale, (0, 0, 0), pose="fighting_stance")

# Weapon on the ground she's lunging toward (a simple cylinder as a pipe/bat)
add_mesh("Weapon", bpy.ops.mesh.primitive_cylinder_add, mat_weapon,
         (0.8, -0.8, 0.05), (0.04, 0.04, 0.4),
         rot=(math.radians(90), 0, math.radians(20)))

# Dead bodies scattered around
build_regular_male("FallenBody1", mat_fallen1, (-1.5, 1.2, 0), pose="fallen")
build_regular_male("FallenBody2", mat_fallen2, (2.0, -1.5, 0), pose="fallen")
build_regular_male("FallenBody3", mat_fallen3, (-2.5, -0.8, 0), pose="fallen")

# Debris — scattered rubble chunks
add_mesh("Debris1", bpy.ops.mesh.primitive_cube_add, mat_debris,
         (1.5, 1.0, 0.1), (0.15, 0.2, 0.1),
         rot=(0, 0, math.radians(35)))
add_mesh("Debris2", bpy.ops.mesh.primitive_cube_add, mat_debris,
         (-0.5, -1.5, 0.08), (0.2, 0.12, 0.08),
         rot=(0, 0, math.radians(-20)))
add_mesh("Debris3", bpy.ops.mesh.primitive_cube_add, mat_debris,
         (2.5, 0.5, 0.06), (0.1, 0.15, 0.06),
         rot=(math.radians(10), 0, math.radians(50)))
add_mesh("Debris4", bpy.ops.mesh.primitive_cylinder_add, mat_debris,
         (-1.0, -2.0, 0.05), (0.05, 0.05, 0.3),
         rot=(math.radians(85), 0, math.radians(60)))

# Camera — eye level medium shot, slightly in front and to the side for action framing
setup_camera(scene, loc=(0.5, -4.0, 1.2), target_loc=(0, 0, 0.9), lens=35)

scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/raider_ambush/layouts/layout_009.png"
