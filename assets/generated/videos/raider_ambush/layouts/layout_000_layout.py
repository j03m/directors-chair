...
mat_truck_cabin = make_mat("TruckCabin", (0.28, 0.2, 0.12, 1))
...
mat_scope_ring = make_mat("ScopeRing", (0.01, 0.01, 0.01, 1))
...
add_mesh("Road", bpy.ops.mesh.primitive_cube_add, mat_road,
         (0, 0, 0.01), (1.8, 12, 0.02))

# ── Truck (beaten-up, stopped on road) ───────────────────────────────────────

# Truck bed
add_mesh("TruckBed", bpy.ops.mesh.primitive_cube_add, mat_truck,
         (0, 1.0, 0.7), (1.2, 1.8, 0.5))
# Truck cabin
add_mesh("TruckCabin", bpy.ops.mesh.primitive_cube_add, mat_truck_cabin,
         (0, 3.2, 0.9), (1.0, 0.8, 0.7))
# Wheels
add_mesh("TruckWheelFL", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-1.1, 2.8, 0.3), (0.3, 0.3, 0.15),
         rot=(0, math.radians(90), 0))
add_mesh("TruckWheelFR", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (1.1, 2.8, 0.3), (0.3, 0.3, 0.15),
         rot=(0, math.radians(90), 0))
add_mesh("TruckWheelRL", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-1.1, 0.0, 0.3), (0.3, 0.3, 0.15),
         rot=(0, math.radians(90), 0))
add_mesh("TruckWheelRR", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (1.1, 0.0, 0.3), (0.3, 0.3, 0.15),
         rot=(0, math.radians(90), 0))

# ── Motorcycles blocking road ────────────────────────────────────────────────

# Motorcycle 1 — front left of truck
add_mesh("Moto1_Body", bpy.ops.mesh.primitive_cube_add, mat_motorcycle,
         (-1.5, 4.5, 0.45), (0.25, 0.7, 0.25),
         rot=(0, 0, math.radians(25)))
add_mesh("Moto1_WheelF", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-1.3, 5.1, 0.25), (0.22, 0.22, 0.08),
         rot=(0, math.radians(90), math.radians(25)))
add_mesh("Moto1_WheelR", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-1.7, 3.9, 0.25), (0.22, 0.22, 0.08),
         rot=(0, math.radians(90), math.radians(25)))

# Motorcycle 2 — front right of truck
add_mesh("Moto2_Body", bpy.ops.mesh.primitive_cube_add, mat_motorcycle,
         (1.8, 4.8, 0.45), (0.25, 0.7, 0.25),
         rot=(0, 0, math.radians(-20)))
add_mesh("Moto2_WheelF", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (1.6, 5.4, 0.25), (0.22, 0.22, 0.08),
         rot=(0, math.radians(90), math.radians(-20)))
add_mesh("Moto2_WheelR", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (2.0, 4.2, 0.25), (0.22, 0.22, 0.08),
         rot=(0, math.radians(90), math.radians(-20)))

# ── Raiders surrounding the truck (fighting_stance — aggressive) ─────────────

# Hockey raider — front left, near motorcycle 1
build_regular_male("hockey", mat_hockey, (-2.2, 3.0, 0), pose="fighting_stance")

# Goggles raider — right side of truck
build_regular_male("goggles", mat_goggles, (2.3, 1.5, 0), pose="fighting_stance")

# Greasy raider — rear of truck
build_regular_male("greasy", mat_greasy, (0.5, -1.2, 0), pose="fighting_stance")

# Heavy raider — front right, near motorcycle 2
build_large_figure("heavy", mat_heavy, (2.5, 3.5, 0), pose="fighting_stance")

# ── Sniper Scope Overlay (close to camera) ──────────────────────────────────

# Large torus ring to simulate scope edge vignette
...
scope_ring.scale = (1.0, 0.1, 1.0)
scope_ring.data.materials.append(mat_scope_ring)

# Crosshair lines (thin cylinders)
add_mesh("CrossH", bpy.ops.mesh.primitive_cylinder_add, mat_crosshair,
         (0, -9.5, 2.5), (0.008, 0.008, 1.6),
         rot=(0, 0, 0))
add_mesh("CrossV", bpy.ops.mesh.primitive_cylinder_add, mat_crosshair,
         (0, -9.5, 2.5), (0.008, 0.008, 1.6),
         rot=(math.radians(90), 0, 0))

# ── Camera — sniper scope POV, distant wide view ────────────────────────────

setup_camera(scene, loc=(0, -12, 3.0), target_loc=(0, 1.5, 0.8), lens=50)

# ── Render ───────────────────────────────────────────────────────────────────

scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/raider_ambush/layouts/layout_000.png"
