...
mat_jeep = make_mat("Jeep", (0.3, 0.32, 0.25, 1))
mat_road = make_mat("Road", (0.12, 0.1, 0.08, 1))
mat_road_line = make_mat("RoadLine", (0.6, 0.55, 0.3, 1))

# Ground plane (desert)
add_ground(mat_ground, size=30)

# Desert road stretching into the distance (long dark strip along Y axis)
add_mesh("Road", bpy.ops.mesh.primitive_cube_add, mat_road,
         (0, 15, 0.01), (2.5, 30, 0.01))

# Center line dashes on the road
for i in range(12):
    y_pos = 2.0 + i * 4.0
    add_mesh(f"RoadLine_{i}", bpy.ops.mesh.primitive_cube_add, mat_road_line,
             (0, y_pos, 0.02), (0.06, 0.8, 0.005))

# Fallen headless zombie in the foreground on the road
# Using "fallen" pose but skipping the head to make it headless
x_z, y_z, z_z = (0.3, 1.5, 0)
add_mesh("Zombie_Body", bpy.ops.mesh.primitive_cube_add, mat_zombie,
         (x_z, y_z, z_z + 0.25), (0.4, 0.3, 0.55),
         rot=(math.radians(80), 0, math.radians(15)))
# No head — headless zombie
add_mesh("Zombie_Arm", bpy.ops.mesh.primitive_cylinder_add, mat_zombie,
         (x_z - 0.5, y_z + 0.4, z_z + 0.1), (0.1, 0.1, 0.4),
         rot=(math.radians(90), 0, math.radians(20)))
add_mesh("Zombie_Leg", bpy.ops.mesh.primitive_cylinder_add, mat_zombie,
         (x_z + 0.4, y_z - 0.5, z_z + 0.08), (0.1, 0.1, 0.4),
         rot=(math.radians(85), 0, math.radians(-25)))
add_mesh("Zombie_LegR", bpy.ops.mesh.primitive_cylinder_add, mat_zombie,
         (x_z + 0.1, y_z - 0.3, z_z + 0.08), (0.1, 0.1, 0.4),
         rot=(math.radians(88), 0, math.radians(-15)))

# Blood pool around the neck area
mat_blood = make_mat("Blood", (0.4, 0.02, 0.02, 1))
add_mesh("BloodPool", bpy.ops.mesh.primitive_cylinder_add, mat_blood,
         (x_z + 0.7, y_z - 0.2, 0.01), (0.3, 0.3, 0.005))

# Military jeep shape driving away in the distance (simple box construction)
jeep_y = 18.0  # far down the road
add_mesh("Jeep_Body", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (0, jeep_y, 0.4), (0.5, 0.8, 0.3))
add_mesh("Jeep_Cab", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (0, jeep_y - 0.3, 0.75), (0.45, 0.4, 0.2))
# Wheels
mat_tire = make_mat("Tire", (0.05, 0.05, 0.05, 1))
add_mesh("Jeep_WheelFL", bpy.ops.mesh.primitive_cylinder_add, mat_tire,
         (-0.55, jeep_y + 0.4, 0.15), (0.15, 0.15, 0.08),
         rot=(0, math.radians(90), 0))
add_mesh("Jeep_WheelFR", bpy.ops.mesh.primitive_cylinder_add, mat_tire,
         (0.55, jeep_y + 0.4, 0.15), (0.15, 0.15, 0.08),
         rot=(0, math.radians(90), 0))
add_mesh("Jeep_WheelRL", bpy.ops.mesh.primitive_cylinder_add, mat_tire,
         (-0.55, jeep_y - 0.5, 0.15), (0.15, 0.15, 0.08),
         rot=(0, math.radians(90), 0))
add_mesh("Jeep_WheelRR", bpy.ops.mesh.primitive_cylinder_add, mat_tire,
         (0.55, jeep_y - 0.5, 0.15), (0.15, 0.15, 0.08),
         rot=(0, math.radians(90), 0))
...
for i in range(3):
    add_mesh(f"Dust_{i}", bpy.ops.mesh.primitive_uv_sphere_add, mat_dust,
             (0.2 * (i - 1), jeep_y - 1.2 - i * 0.8, 0.2 + i * 0.15),
             (0.3 + i * 0.15, 0.3 + i * 0.1, 0.15 + i * 0.05))

# Camera: ground level, low angle, wide lens, looking past the body toward the jeep
setup_camera(scene,
             loc=(0.5, -1.0, 0.25),
             target_loc=(0, 12.0, 0.5),
             lens=24)

# Render
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/desert_run_dead_zombie/layouts/layout_002.png"
