...
mat_jeep = make_mat("Jeep", (0.25, 0.22, 0.18, 1))
mat_bat = make_mat("Bat", (0.4, 0.3, 0.15, 1))

# Ground
add_ground(mat_ground)

# ── Jeep (simple box shape for the rear bed) ────────────────────────────────
# Jeep bed — flat platform the gorilla stands on, moving forward (+Y direction)
add_mesh("Jeep_Bed", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (0, 0, 0.4), (1.2, 1.8, 0.15))
# Jeep side walls
add_mesh("Jeep_SideL", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (-1.2, 0, 0.7), (0.08, 1.8, 0.25))
add_mesh("Jeep_SideR", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (1.2, 0, 0.7), (0.08, 1.8, 0.25))
# Jeep tailgate (rear, behind gorilla)
add_mesh("Jeep_Tail", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (0, -1.8, 0.7), (1.2, 0.08, 0.25))
# Jeep cab (front portion, ahead of the bed)
add_mesh("Jeep_Cab", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (0, 2.5, 1.0), (1.1, 0.8, 0.5))
# Wheels
mat_wheel = make_mat("Wheel", (0.1, 0.1, 0.1, 1))
add_mesh("Wheel_FL", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-1.3, 1.5, 0.2), (0.25, 0.25, 0.12),
         rot=(0, math.radians(90), 0))
add_mesh("Wheel_FR", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (1.3, 1.5, 0.2), (0.25, 0.25, 0.12),
         rot=(0, math.radians(90), 0))
add_mesh("Wheel_RL", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-1.3, -1.0, 0.2), (0.25, 0.25, 0.12),
         rot=(0, math.radians(90), 0))
add_mesh("Wheel_RR", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (1.3, -1.0, 0.2), (0.25, 0.25, 0.12),
         rot=(0, math.radians(90), 0))

# ── Gorilla — foreground left, fighting stance, swinging bat ─────────────────
# Gorilla is in rear of jeep bed, left side, facing right toward zombie
build_large_figure("Gorilla", mat_gorilla, (-0.4, -0.3, 0.55), pose="fighting_stance")

# Baseball bat — extending from gorilla's right arm toward the zombie
add_mesh("Bat", bpy.ops.mesh.primitive_cylinder_add, mat_bat,
         (0.6, -0.8, 2.1), (0.06, 0.06, 0.7),
         rot=(math.radians(-30), math.radians(50), math.radians(20)))

# ── Zombie — at arm's length to the right, at head height ────────────────────
# Zombie is outside the jeep on the right side, at the gorilla's head level
# Positioned at arm's length — roughly 1.5-2m to the right and slightly forward
build_regular_male("Zombie", mat_zombie, (2.0, 0.2, 0.3), pose="standing")

# ── Camera — tight over-the-shoulder behind gorilla's right shoulder ─────────
# Camera behind and slightly right of gorilla, looking past bat toward zombie
setup_camera(scene,
             loc=(0.3, -2.2, 2.8),
             target_loc=(1.8, 0.2, 2.0),
             lens=24)

# ── Render ───────────────────────────────────────────────────────────────────
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/desert_run_dead_zombie/layouts/layout_001.png"
