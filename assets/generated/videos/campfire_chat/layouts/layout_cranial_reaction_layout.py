...
mat_chair = make_mat("Chair", (0.12, 0.08, 0.05, 1))
...
# Left chair (for Cranial) — low seat + back
add_mesh("ChairL_Seat", bpy.ops.mesh.primitive_cube_add, mat_chair,
         (-1.3, 3.0, 0.25), (0.5, 0.4, 0.04))
add_mesh("ChairL_Back", bpy.ops.mesh.primitive_cube_add, mat_chair,
         (-1.3, 3.35, 0.55), (0.5, 0.04, 0.3),
         rot=(math.radians(-15), 0, 0))

# Right chair (for Gorilla) — slightly wider
add_mesh("ChairR_Seat", bpy.ops.mesh.primitive_cube_add, mat_chair,
         (1.3, 3.0, 0.3), (0.65, 0.5, 0.05))
add_mesh("ChairR_Back", bpy.ops.mesh.primitive_cube_add, mat_chair,
         (1.3, 3.45, 0.65), (0.65, 0.05, 0.35),
         rot=(math.radians(-15), 0, 0))

# ── Characters ───────────────────────────────────────────────────────────────

# Cranial — seated on left chair
build_regular_male("Cranial", mat_cranial, (-1.3, 3.0, 0.25), pose="seated")

# Gorilla — seated on right chair (large figure)
build_large_figure("Gorilla", mat_gorilla, (1.3, 3.0, 0.3), pose="seated")

# Robot — standing in front of the two, facing them, closer to camera
build_regular_male("Robot", mat_robot, (0.0, 1.2, 0.0), pose="standing")

# ── Camera ───────────────────────────────────────────────────────────────────

# Low angle, slightly below eye level, ~6 feet away, showing full bodies
setup_camera(scene, loc=(0.0, -0.6, 0.65), target_loc=(0.0, 2.2, 1.0), lens=28)

# ── Render ───────────────────────────────────────────────────────────────────

scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/campfire_chat/layouts/layout_cranial_reaction.png"
