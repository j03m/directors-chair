...
mat_jeep = make_mat("Jeep", (0.18, 0.22, 0.15, 1))
...
# Road — long flat strip down the Y axis
add_mesh("Road", bpy.ops.mesh.primitive_cube_add, mat_road,
         (0, 15, 0.01), (2.5, 30, 0.01))

# Road center line dashes
for i in range(12):
    add_mesh(f"RoadLine_{i}", bpy.ops.mesh.primitive_cube_add,
             make_mat("LineYellow", (0.7, 0.6, 0.1, 1)),
             (0, i * 5, 0.02), (0.05, 0.8, 0.005))

# ── Jeep ─────────────────────────────────────────────────────────────────────
# Jeep positioned on left side of road, facing forward (+Y)
jeep_x, jeep_y = -1.2, 2.0

# Jeep body — main chassis
add_mesh("Jeep_Body", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x, jeep_y, 0.6), (1.0, 1.8, 0.35))

# Jeep hood (lower front section)
add_mesh("Jeep_Hood", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x, jeep_y + 1.6, 0.5), (0.85, 0.6, 0.2))

# Jeep windshield frame
add_mesh("Jeep_Windshield", bpy.ops.mesh.primitive_cube_add, mat_windshield,
         (jeep_x, jeep_y + 0.9, 1.1), (0.85, 0.05, 0.3))

# Jeep roll cage / rear frame
add_mesh("Jeep_RollBar", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x, jeep_y - 0.8, 1.1), (0.9, 0.05, 0.35))

# Rear cargo bed walls
add_mesh("Jeep_SideL", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x - 0.95, jeep_y - 0.5, 0.85), (0.05, 0.8, 0.2))
add_mesh("Jeep_SideR", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x + 0.95, jeep_y - 0.5, 0.85), (0.05, 0.8, 0.2))
add_mesh("Jeep_Tailgate", bpy.ops.mesh.primitive_cube_add, mat_jeep,
         (jeep_x, jeep_y - 1.3, 0.85), (0.9, 0.05, 0.2))

# Wheels
for wx, wy in [(-0.85, 1.2), (0.85, 1.2), (-0.85, -0.6), (0.85, -0.6)]:
    add_mesh(f"Wheel_{wx}_{wy}", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
             (jeep_x + wx, jeep_y + wy, 0.25), (0.25, 0.25, 0.12),
             rot=(math.radians(90), 0, 0))

# ── Characters in Jeep ──────────────────────────────────────────────────────

# Cranial — driver, front left, seated
build_regular_male("Cranial", mat_cranial,
                   (jeep_x - 0.4, jeep_y + 0.3, 0.55), pose="seated")

# Robot — passenger, front right, seated
build_regular_male("Robot", mat_robot,
                   (jeep_x + 0.4, jeep_y + 0.3, 0.55), pose="seated")

# Gorilla — standing in rear cargo bed, arms raised (holding bat overhead)
build_large_figure("Gorilla", mat_gorilla,
                   (jeep_x, jeep_y - 0.6, 0.65), pose="arms_raised")

# Bat held by gorilla — long cylinder above gorilla's hands
add_mesh("Bat", bpy.ops.mesh.primitive_cylinder_add, mat_bat,
         (jeep_x, jeep_y - 0.6, 3.4), (0.06, 0.06, 0.7),
         rot=(0, math.radians(15), math.radians(5)))

# ── Zombie — far ahead on right shoulder ─────────────────────────────────────
build_regular_male("Zombie", mat_zombie,
                   (2.0, 18.0, 0.0), pose="standing")

# ── Camera ───────────────────────────────────────────────────────────────────
# Behind and slightly above the jeep, looking forward past it toward the zombie
setup_camera(scene,
             loc=(jeep_x - 0.5, jeep_y - 5.5, 3.0),
             target_loc=(0, 14.0, 1.0),
             lens=35)

# ── Render ───────────────────────────────────────────────────────────────────
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/zombie_driveup/layouts/layout_000.png"
