...
mat_zombie = make_mat("Zombie", (0.25, 0.18, 0.1, 1))
mat_road = make_mat("Road", (0.12, 0.12, 0.12, 1))
mat_road_line = make_mat("RoadLine", (0.6, 0.55, 0.2, 1))

# Ground plane (desert)
add_ground(mat_ground, size=15)

# Road surface - long flat cube to simulate highway
...
for i in range(-12, 13, 3):
    add_mesh(f"RoadLine_{i}", bpy.ops.mesh.primitive_cube_add, mat_road_line,
             (0, i, 0.02), (0.05, 0.5, 0.005))

# Road shoulder lines
add_mesh("ShoulderL", bpy.ops.mesh.primitive_cube_add, mat_road_line,
         (-1.8, 0, 0.02), (0.03, 15.0, 0.005))
add_mesh("ShoulderR", bpy.ops.mesh.primitive_cube_add, mat_road_line,
         (1.8, 0, 0.02), (0.03, 15.0, 0.005))

# Place zombie on right shoulder, shambling forward (standing pose, slouched via body tilt)
build_regular_male("Zombie", mat_zombie, (2.5, 3.0, 0.0), pose="standing")

# Tilt the zombie body forward slightly for shambling gait
zombie_body = bpy.data.objects.get("Zombie_Body")
if zombie_body:
    zombie_body.rotation_euler = (math.radians(15), 0, math.radians(-5))

# Tilt head down for slouched look
zombie_head = bpy.data.objects.get("Zombie_Head")
if zombie_head:
    zombie_head.rotation_euler = (math.radians(20), 0, math.radians(-5))

# Camera on road surface looking toward zombie on the right
setup_camera(scene, loc=(0.0, -4.0, 1.2), target_loc=(2.5, 3.0, 1.0), lens=50)

scene.frame_set(1)

output_path = "/Users/jmordetsky/directors-chair/assets/generated/videos/desert_run_zombie/layouts/layout_000.png"
