...
add_mesh("ChairL_Seat", bpy.ops.mesh.primitive_cube_add, mat_chair,
         (-1.5, 0.0, 0.25), (0.5, 0.35, 0.05),
         rot=(math.radians(-10), 0, 0))
add_mesh("ChairL_Back", bpy.ops.mesh.primitive_cube_add, mat_chair,
         (-1.5, 0.35, 0.55), (0.5, 0.05, 0.3),
         rot=(math.radians(-15), 0, 0))
add_mesh("ChairL_LegFL", bpy.ops.mesh.primitive_cylinder_add, mat_chair,
         (-1.9, -0.25, 0.12), (0.03, 0.03, 0.15))
add_mesh("ChairL_LegFR", bpy.ops.mesh.primitive_cylinder_add, mat_chair,
         (-1.1, -0.25, 0.12), (0.03, 0.03, 0.15))
add_mesh("ChairL_LegBL", bpy.ops.mesh.primitive_cylinder_add, mat_chair,
         (-1.9, 0.3, 0.2), (0.03, 0.03, 0.22))
add_mesh("ChairL_LegBR", bpy.ops.mesh.primitive_cylinder_add, mat_chair,
         (-1.1, 0.3, 0.2), (0.03, 0.03, 0.22))

# Right chair (for gorilla — wider)
add_mesh("ChairR_Seat", bpy.ops.mesh.primitive_cube_add, mat_chair,
         (1.2, 0.0, 0.3), (0.65, 0.4, 0.05),
         rot=(math.radians(-10), 0, 0))
add_mesh("ChairR_Back", bpy.ops.mesh.primitive_cube_add, mat_chair,
         (1.2, 0.4, 0.65), (0.65, 0.05, 0.35),
         rot=(math.radians(-15), 0, 0))
add_mesh("ChairR_LegFL", bpy.ops.mesh.primitive_cylinder_add, mat_chair,
         (0.65, -0.3, 0.15), (0.04, 0.04, 0.17))
add_mesh("ChairR_LegFR", bpy.ops.mesh.primitive_cylinder_add, mat_chair,
         (1.75, -0.3, 0.15), (0.04, 0.04, 0.17))
add_mesh("ChairR_LegBL", bpy.ops.mesh.primitive_cylinder_add, mat_chair,
         (0.65, 0.35, 0.25), (0.04, 0.04, 0.25))
add_mesh("ChairR_LegBR", bpy.ops.mesh.primitive_cylinder_add, mat_chair,
         (1.75, 0.35, 0.25), (0.04, 0.04, 0.25))

# Characters
# Cranial — seated in left chair
build_regular_male("Cranial", mat_cranial, (-1.5, 0.0, 0.0), pose="seated")

# Gorilla — mid-stand, pushing up from right chair (arms_raised to show pushing up)
build_large_figure("Gorilla", mat_gorilla, (1.2, 0.0, 0.0), pose="arms_raised")

# Robot — standing to the right of the chairs
build_regular_male("Robot", mat_robot, (3.2, 0.0, 0.0), pose="standing")

# Camera — wide shot, eye level, about 8 feet (2.4m) away, facing the group
setup_camera(scene, loc=(0.5, -5.0, 1.2), target_loc=(0.5, 0.0, 1.0), lens=28)

# Render
scene.frame_set(1)
scene.render.filepath = "/Users/jmordetsky/directors-chair/assets/generated/videos/sniper_cliff/layouts/layout_000.png"
