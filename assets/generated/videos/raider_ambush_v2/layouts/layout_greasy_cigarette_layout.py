...
mat_truck = make_mat("Truck", (0.35, 0.28, 0.2, 1))
mat_truck_dark = make_mat("TruckDark", (0.18, 0.15, 0.12, 1))
...
# Road stretching into the distance along +Y axis
add_mesh("Road", bpy.ops.mesh.primitive_cube_add, mat_road,
         (0, 20, 0.005), (1.8, 40, 0.005))

# Road shoulder lines
mat_shoulder = make_mat("Shoulder", (0.15, 0.12, 0.09, 1))
add_mesh("ShoulderL", bpy.ops.mesh.primitive_cube_add, mat_shoulder,
         (-1.9, 20, 0.006), (0.08, 40, 0.005))
add_mesh("ShoulderR", bpy.ops.mesh.primitive_cube_add, mat_shoulder,
         (1.9, 20, 0.006), (0.08, 40, 0.005))

# ── Foreground figure: Cranial, right side, standing ─────────
build_regular_male("Cranial", mat_cranial, (1.5, 2.0, 0), pose="standing")

# ── Background: stopped truck far down the road ─────────────
# Truck bed / cargo container
add_mesh("TruckBed", bpy.ops.mesh.primitive_cube_add, mat_truck,
         (0, 30, 1.0), (1.2, 2.5, 1.0))
# Truck cab
add_mesh("TruckCab", bpy.ops.mesh.primitive_cube_add, mat_truck_dark,
         (0, 27.0, 0.8), (1.0, 1.0, 0.8))
# Truck wheels (simple dark cylinders)
mat_wheel = make_mat("Wheel", (0.05, 0.05, 0.05, 1))
add_mesh("WheelFL", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-1.1, 26.5, 0.3), (0.3, 0.3, 0.15),
...
add_mesh("WheelRL", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-1.1, 31.0, 0.3), (0.3, 0.3, 0.15),
         rot=(math.radians(90), 0, 0))
add_mesh("WheelRR", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (1.1, 31.0, 0.3), (0.3, 0.3, 0.15),
         rot=(math.radians(90), 0, 0))

# ── Small figures near the truck in the distance ─────────────
build_regular_male("DistNomad1", mat_nomad1, (-2.0, 28.0, 0), pose="standing")
build_regular_male("DistNomad2", mat_nomad2, (2.2, 30.0, 0), pose="standing")
build_regular_male("DistNomad3", mat_nomad3, (-1.0, 32.0, 0), pose="standing")

# ── Camera: behind and slightly to the side, medium shot ─────
# Camera positioned behind-right of Cranial, looking past him down the road
setup_camera(scene,
             loc=(3.0, -0.5, 1.4),
             target_loc=(0.2, 18, 0.9),
             lens=32)
