...
mat_nomad_group = make_mat("NomadGroup", (0.4, 0.35, 0.3, 1))

# Ground
add_ground(mat_ground, size=25)

# ── Box Truck (center-left, camera shoots from behind it) ────────────────────

# Truck cargo box — large rectangular box
add_mesh("Truck_CargoBox", bpy.ops.mesh.primitive_cube_add, mat_truck,
         (0, 2.0, 1.8), (1.4, 2.5, 1.8))

# Truck cab — smaller box at front
add_mesh("Truck_Cab", bpy.ops.mesh.primitive_cube_add, mat_truck_dark,
         (0, 5.5, 1.2), (1.2, 1.2, 1.2))

# Truck wheels
mat_wheel = make_mat("Wheel", (0.05, 0.05, 0.05, 1))
add_mesh("Truck_WheelBL", bpy.ops.mesh.primitive_cylinder_add, mat_wheel,
         (-1.5, 0.5, 0.4), (0.4, 0.4, 0.15),
...
# Truck undercarriage
add_mesh("Truck_Chassis", bpy.ops.mesh.primitive_cube_add, mat_truck_dark,
         (0, 3.0, 0.35), (1.1, 3.5, 0.2))

# ── Heavy (crouching behind truck, left side) ────────────────────────────────
# Crouching = seated pose, positioned at rear-left of truck, partially hidden
build_large_figure("Heavy", mat_heavy, (-1.8, 0.5, 0), pose="seated")

# ── Distant nomad group (right of truck, far down the road) ──────────────────
# Small figures visible in the distance to the right, gathered on the road
# Using small scale trick: place them far away so they read as distant

build_regular_male("Nomad_A", mat_nomad_group, (3.5, 18.0, 0), pose="standing")
build_regular_male("Nomad_B", mat_nomad_group, (4.5, 18.5, 0), pose="standing")
build_regular_male("Nomad_C", mat_nomad_group, (4.0, 19.5, 0), pose="standing")
build_regular_female("Nomad_D", mat_nomad_group, (3.0, 19.0, 0), pose="standing")

# ── Camera ───────────────────────────────────────────────────────────────────
# Medium shot from behind the truck, looking past it down the road
# Camera is behind and slightly to the right to see both the heavy and the road
setup_camera(scene,
             loc=(1.5, -3.5, 2.0),
             target_loc=(1.0, 8.0, 1.2),
             lens=32)
