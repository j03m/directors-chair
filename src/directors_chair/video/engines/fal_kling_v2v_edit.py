...
def edit_clip(
    ...
    """Edit an existing video clip via Kling O1 Video-to-Video Edit.
    ...
    console.print(f"  [dim]Video uploaded ({os.path.getsize(video_path) // 1024}KB)[/dim]")
    ...
    for attempt in range(max_retries):
        try:
            with console.status("[cyan]Editing video via Kling O1 v2v...[/cyan]") as status:
